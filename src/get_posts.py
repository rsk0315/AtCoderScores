...
## This script is incomplete; tsurai
...
'''.encode('utf-8')
    ...
    for score, problems in sorted(list(ss.items())):
        ...
        for contest_type in Problem.CONTEST_TYPES:
          ...
          <td id="{task_bname}_task"><a href="{task_url}">{task_name}</a></td>
          <td id="{task_bname}_writer">{writers}</td>
          <td id="{task_bname}_pscore">{partial_score}</td>
        </tr>'''.encode('utf-8')
                ).format(
                    score=score,
